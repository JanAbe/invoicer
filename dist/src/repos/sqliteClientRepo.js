    ...
    SqliteClientRepo.prototype.clientOfID = function (clientID) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        query = 'SELECT id, firstName, lastName, email, city, street, houseNumber, zipcode FROM Client WHERE id = ?;';
                        return [4 /*yield*/, this._db.get(query, [clientID.toString()])];
                    case 1:
                        row = _a.sent();
                        return [2 /*return*/, new client_1.Client(new clientID_1.ClientID(row.id), new fullName_1.FullName(row.firstName, row.lastName), new email_1.Email(row.email), new address_1.Address(row.city, row.street, row.houseNumber, row.zipcode))];
                }
            });
        });
    };
