...
var jobDTO_1 = require("../../domain/dto/jobDTO");
                        ...
                        return [4 /*yield*/, new Promise(function (resolve, reject) {
                                _this._db.db.get(cameramanQuery, jobID.toString(), function (err, row) {
                                    if (err) {
                                        ...
                                        if (row !== undefined) {
                                            var cameraman = new cameraman_1.Cameraman(row.firstName, row.lastName, row.day_price, new period_1.Period(moment(row.start_date, 'DD/MM/YYYY').toDate(), moment(row.end_date, 'DD/MM/YYYY').toDate()));
                                    ...
                                    if (err) {
                                        ...
                                        if (rows !== undefined) {
                                            rows.forEach(function (row) {
                                                var equipmentItem = new equipmentItem_1.EquipmentItem(row.name, row.day_price, new period_1.Period(moment(row.start_date, 'DD/MM/YYYY').toDate(), moment(row.end_date, 'DD/MM/YYYY').toDate()));
